		...
		try {
			...
			if (listaConcerns.size() == 0) {
				...
				for (CKNumber classMetrics : report.all()) {
					writer.write(classMetrics.getClassName(), "\"" + classMetrics.getDesignRole() + "\"", classMetrics.isArchitecturalRole());
				}
			} else {
				...
				for (CKNumber classMetrics : report.all()) {
					if (listaConcerns.contains(classMetrics.getDesignRole())) {
						writer.write(applicationName, classMetrics.getClassName(), "\"" + classMetrics.getDesignRole() + "\"", classMetrics.isArchitecturalRole());
					}
				}
			}
