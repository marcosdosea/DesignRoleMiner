...
//				.process(new MethodVisitorCK(), new CSVFile("D:/Projetos/_Web/method-web-budget-metrics.csv")).mine();
	...
	private void mineVersions() {
		...
		List<String> androidConcerns = Arrays.asList("activity", "service");
		new RepositoryMining().in(GitRepository.singleProject("D:/Projetos/_Android/k-9")).through(Commits.list(k9Android))
			.withThreads(5)
			.process(new MethodVisitorCK(androidConcerns), new CSVFile("D:/Projetos/_Android/versions-method-k-9-metrics.csv")).mine();

		List<String> eclipseConcerns = Arrays.asList("dialog", "view");
		...
		new RepositoryMining().in(GitRepository.singleProject("D:/Projetos/_Eclipse/droolsjbpm-tools"))
			...
			.process(new MethodVisitorCK(eclipseConcerns), new CSVFile("D:/Projetos/_Eclipse/versions-method-droolsjbpm-tools-metrics.csv"))
			.mine();

		List<String> webConcerns = Arrays.asList("persistence", "service");
		...
		new RepositoryMining().in(GitRepository.singleProject("D:/Projetos/_Web/openmrs-core"))
			...
			.process(new MethodVisitorCK(webConcerns), new CSVFile("D:/Projetos/_Web/versions-method-openmrs-core-metrics.csv"))
			.mine();
	}
